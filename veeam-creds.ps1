...
try {
	$VeaamRegPath = "HKLM:\SOFTWARE\Veeam\Veeam Endpoint Backup\"
    $key = "hklm:\SOFTWARE\Veeam\Veeam Endpoint Backup\DbConfiguration\"
    $user = (Get-ItemProperty -Path $key -Name SqlLogin).SqlLogin 
    $passwd =(Get-ItemProperty -Path $key -Name SqlPassword).SqlPassword 
    $instance =(Get-ItemProperty -Path $key -Name SqlInstancePipeName).SqlInstancePipeName 
...
"Initial Catalog=$SqlDatabaseName; User ID=$user;Password=$passwd "
